...
async function fetchProfile(key) {
	  ...
	  FOR p IN Persons
	        ...
	        gender: p.gender, maidenName: p.maidenName, born: p.born, died: p.died, pic: p.pic, info: p.info },
          {
              rod: FIRST(FOR rod IN Rods
                      ...
                      RETURN {name: rod.name, _id: rod._id, _key: rod._key}),
  ...
  if (profile === undefined) ctx.throw(404, 'person not found');
  return profile
}
...
async function findClosestUsers(fromId) {
  ...
  const relUsers = await db.query(
    aql`FOR v, e, p
          ...
          FILTER v.user.status == 1
          FILTER p.edges[*].del ALL == null
        RETURN {_id: v._id, name: v.name, surname: v.surname}`)
    .then(cursor => cursor.all());

  return relUsers
}

module.exports = {fetchPerson, fetchProfile, getPredki, getPotomki, fetchPredkiPotomki,
