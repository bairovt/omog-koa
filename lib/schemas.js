...
exports.personSchema = Joi.object().keys({
  ...
  surname: nameSchema.empty('').default(undefined),
  midname: nameSchema.empty('').default(undefined),
  maidenName: nameSchema.empty('').default(undefined),
  info: Joi.string().trim().min(1).max(1000).empty('').default(undefined),
  lifestory: textSchema.empty('').default(undefined),
  rod: Joi.string().max(100).allow(null), // allow('') is same: empty('').default('')
  gender: Joi.number().integer().min(0).max(1).required(),
  born: Joi.number().integer().min(0).max(2020).empty(''),
  died: Joi.number().integer().min(0).max(2020).empty(''),
});
